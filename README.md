...
### 智能体（Agent）
- **添加 MCP**：设置页顶部的「MCP 服务」支持添加、编辑、启停、删除与连接测试。填写 Streamable HTTP 服务地址（通常为 `/mcp`），可附带 JSON 请求头，例如 `{"Authorization":"Bearer YOUR_TOKEN"}`。配置在本机加密保存，启动应用或修改配置后自动加载工具，供智能体和问答按需调用；暂不支持本地 stdio 命令和旧版 SSE 端点。智能体仍不使用通用网页搜索工具。
- 屏幕查看：MediaProjection 真实像素截图
- 界面理解：ML Kit OCR + AccessibilityService 控件树解析
- 自动操作：打开应用 / 点击 / 双击 / 长按 / 输入 / 滑动 / 滚动 / 返回 / 桌面 / 最近任务 / 等待 / 截图
- **按需调用接口**：天气 / 新闻 / 汇率 / 加密货币 / 股票行情 / 金银价 / 维基百科 / 节假日 / 路线距离耗时 / 歌曲信息 / 歌词 / IP 归属地 / 世界时间 / 空气质量 / 书籍 / 英语词典通过专用接口获取；智能体工作期间不进行通用联网搜索，接口失败或未安装购物应用时也不会自动转为网页搜索。问答模式的联网搜索不受影响。
...
> 本项目内置自研 `ConeSearch`：**本地 FTS 优先 + Bing 联网兜底** 的 RAG 检索引擎，为问答的 `web_search` / `fetch` / `index_url` 三工具提供上下文；智能体执行任务时不使用通用联网搜索。
